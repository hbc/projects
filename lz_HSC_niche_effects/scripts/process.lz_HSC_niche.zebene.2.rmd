...
pd.gvis <- gvisTable(as.data.frame(apply(pData(affyRaw), 2, as.character)), options=list(width=960))  
print(pd.gvis, "chart")
```
...
The new arrays look fine. There is still good clustering by cell type. There doesn't appear to be too much clustering by batch (batch 5 are the new arrays).
...
I kept the old lyrcorine samples in the analysis for now, to compare with the new samples.
...
**Including old lycorine samples.**

*All Samples*
```{r basic_clustering1, out.width='33%', fig.show='hold'}
plot_dendro(affyNorm, labels.colname="celltype", colors.colname="treatment", title="Cell Types and Treatments, All Samples - All Lycorine Concentrations")
```

*Subset by Cell Type*
```{r basic_clustering2, out.width='33%', fig.show='hold'}
...
plot_dendro(affyNorm[,affyNorm@phenoData$celltype=="ENDO"], labels.colname="treatment", colors.colname="Batch", title="Treatments and Batches, ENDO Cells - All Lycorine Concentrations")
plot_dendro(affyNorm[,affyNorm@phenoData$celltype=="HSC"], labels.colname="treatment", colors.colname="Batch", title="Treatments and Batches, HSC Cells - All Lycorine Concentrations")
```

**After exluding old lycorine samples.**
*All Samples*
```{r basic_clustering3, out.width='33%', fig.show='hold'}
plot_dendro(affyNorm[,which(pData(affyNorm)$lycorine_conc!="LO")], labels.colname="celltype", colors.colname="treatment", title="Cell Types and Treatments, All Samples - High Lycorine Concentration")


```

*Subset by Cell Type*
```{r basic_clustering4, out.width='33%', fig.show='hold'}
plot_dendro(affyNorm[,which(affyNorm@phenoData$celltype=="NEG" & affyNorm@phenoData$lycorine_conc!="LO")], labels.colname="treatment", colors.colname="Batch", title="Treatments and Batches, NEG Cells - High Lycorine Concentration")
plot_dendro(affyNorm[,which(affyNorm@phenoData$celltype=="ENDO" & affyNorm@phenoData$lycorine_conc!="LO")], labels.colname="treatment", colors.colname="Batch", title="Treatments and Batches, ENDO Cells - High Lycorine Concentration")
plot_dendro(affyNorm[,which(affyNorm@phenoData$celltype=="HSC" & affyNorm@phenoData$lycorine_conc!="LO")], labels.colname="treatment", colors.colname="Batch", title="Treatments and Batches, HSC Cells - High Lycorine Concentration")
```

The endothelial cells are clustering together well. There is still less separation between the HSC and negative cells. There is definite improvement of clustering based on treatment, if you exclude the old lycorine samples. There does not appear to be any overwhelming clustering based on batch (i.e. the samples do not mainly separate based on batch). There may be some effects as subgroups of celltypes do appear to form subgroups based on batch, but that may be based on the lycorine treatment instead of batch. In any case, we will batch correct. 

Going forward, I excluded the lower concentration lycorine samples.
...
*All samples*
```{r pca1, out.width='33%', fig.show='hold'}
PCAplot(affyNorm, categories="celltype", colorpalette=cbPalette[1:3], alpha=0.8, title="All Samples - Cell Type")
PCAplot(affyNorm, categories="treatment", colorpalette=cbPalette[5:7], alpha=0.8, title="All Samples - Treatment")
```

*Subset by Treatment*
```{r pca2, out.width='33%', fig.show='hold'}
PCAplot(affyNorm[,which(affyNorm@phenoData$treatment=="DMSO")], categories="celltype", colorpalette=cbPalette[1:3], alpha=0.8, title="DMSO Treatment - Cell Types")
PCAplot(affyNorm[,which(affyNorm@phenoData$treatment=="PGE2")], categories="celltype", colorpalette=cbPalette[1:3], alpha=0.8, title="PGE2 Treatment - Cell Types")
PCAplot(affyNorm[,which(affyNorm@phenoData$treatment=="Lycorine")], categories="celltype", colorpalette=cbPalette[1:3], alpha=0.8, title="Lycorine Treament - Cell Types")
```

*Subset by Cell Type*
```{r pca3, out.width='33%', fig.show='hold'}
PCAplot(affyNorm[,which(affyNorm@phenoData$celltype=="HSC")], categories="treatment", colorpalette=cbPalette[5:7], alpha=0.8, title="HSC Cells - Treatments")
PCAplot(affyNorm[,which(affyNorm@phenoData$celltype=="ENDO")], categories="treatment", colorpalette=cbPalette[5:7], alpha=0.8, title="ENDO Cells - Treatments")
PCAplot(affyNorm[,which(affyNorm@phenoData$celltype=="HSC")], categories="treatment", colorpalette=cbPalette[5:7], alpha=0.8, title="NEG Cells - Treatments")
```

**Principal Component Analysis Figures**
The figures shows a scatterplot of the arrays along all dual combinations of the first four principal components. Each sample group is represented by a separate color.  For the first plot, the sample types represent cell types. In the second  plot, the sample types represent the three treatments. In the first set of three plots, each plot is  for a single treatment, and the sample groups (colors) represent cell types.In the second set of three plots, each plot is  for a cel type, and the sample groups (colors) represent treatments.
...
Similar to the dendrogram results it appears that the samples cluster fairly well by cell type, and somewhat by drug treatment.   
...
*All Samples*
```{r batch.basic_clustering1, out.width='33%', fig.show='hold'}
plot_dendro(affyNorm.batch[,which(pData(affyNorm.batch)$lycorine_conc!="LO")], labels.colname="celltype", colors.colname="treatment", title="Cell Types and Treatments, Batch Corrected Samples - High Lycorine Concentration")
```

*Subset by Cell Type*
```{r batch.basic_clustering2, out.width='33%', fig.show='hold'}
plot_dendro(affyNorm.batch[,which(affyNorm.batch@phenoData$celltype=="NEG" & affyNorm.batch@phenoData$lycorine_conc!="LO")], labels.colname="treatment", colors.colname="Batch", title="Treatments and Batches, NEG Cells - High Lycorine Concentration")
plot_dendro(affyNorm.batch[,which(affyNorm.batch@phenoData$celltype=="ENDO" & affyNorm.batch@phenoData$lycorine_conc!="LO")], labels.colname="treatment", colors.colname="Batch", title="Treatments and Batches, ENDO Cells - High Lycorine Concentration")
plot_dendro(affyNorm.batch[,which(affyNorm.batch@phenoData$celltype=="HSC" & affyNorm.batch@phenoData$lycorine_conc!="LO")], labels.colname="treatment", colors.colname="Batch", title="Treatments and Batches, HSC Cells - High Lycorine Concentration")
```

### Post Batch correction PCA

*All samples*
```{r pca.batch1, out.width='33%', fig.show='hold'}
...
*Subset cell type*
```{r pca.batch2, out.width='33%', fig.show='hold'}
...
*Subset by treatment*
```{r pca.batch3, out.width='33%', fig.show='hold'}
PCAplot(affyNorm.batch[,which(affyNorm.batch@phenoData$treatment=="DMSO")], categories="celltype", colorpalette=cbPalette[1:3], alpha=0.8, title="DMSO Treatment - Cell Types")
PCAplot(affyNorm.batch[,which(affyNorm.batch@phenoData$treatment=="PGE2")], categories="celltype", colorpalette=cbPalette[1:3], alpha=0.8, title="PGE2 Treatment - Cell Types")
PCAplot(affyNorm.batch[,which(affyNorm.batch@phenoData$treatment=="Lycorine")], categories="celltype", colorpalette=cbPalette[1:3], alpha=0.8, title="Lycorine Treament - Cell Types")
```

Clear improvement in cell type separation after batch correction.
Slight improvement of treatment clustering after batch correction.

## Pre-filtering

Now that we're done normalizing, we can get rid of the control probes (nearly 2/3 of the probes on the array).
...
design.gvis <- gvisTable(as.data.frame(apply(design, 2, as.character)), options=list(width=960))  
...
contrast.gvis <- gvisTable(as.data.frame(apply(contrast.matrix, 2, as.character)), options=list(width=960,height=320 ))  
...
```{r volcanoplot, fig.cap="", fig.width=12, fig.height=12, out.width='100%', fig.show='hold', results='hide', cache=TRUE, dev='png'}
...
[Links to Excel files with results for genes in `r all.stats[[index]]$contrast` comparison](./results/hilyco/`r paste("all.stats", all.stats[[index]]$contrast, "xls",sep=".")`)
`r index=index+1`

[Links to Excel file with results for genes in `r all.stats[[index]]$contrast` comparison](./results/hilyco/`r paste("all.stats", all.stats[[index]]$contrast, "xls",sep=".")`)
`r index=index+1`
...
```{r heatmaps, fig.width=16, fig.height=16, eval=F}
