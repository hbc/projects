---
title: "Quality Control - Two lung blood study"
...
interestingGroups(bcb) <- c("lpsLevel", "timepointSampled")
...
Total number of reads looks great.
...
The mapping rate is good, though may be too low in low_lps_9_24hr and high_lps_1_6hr.
...
We are detecting a good number of genes in all samples.
...
Here' the exonic mapping rates look pretty low, though as previously discussed this may be because of the poor annotation of the sheep genome (i.e. reads mapping to real genes may be seen as genomic in an incomplete annotation)
...
The reads taht aren't mapping to exons are not (excepting maybe low_lps_4_0hr) generally mapping to introns, which is suggests supports the theory about incomplete annotation causing low exonic rates.
...
Samples should have a ribosomal RNA (rRNA) contamination rate below 10%. The rates here are terrific, as one would expect for polyA RNA.
...
There is little 5'->3' bias, suggesting little to no presence of RNA degradation.
...
All the samples look fairly similar here.
...
With the exception of high_lps_1_0hr, the samples cluster fairly well by class, with the first principal component capturing treatment and the second capturing lps treatment.
...
Before continuing, I dropped the high_lps_1_6hr and low_lps_9_24hr samples due to their low mapping rates.

```{r bsbsubset}
mycoldata <- as.data.frame(colData(bcb))

bcb_sub <- bcb[,which(!mycoldata$sampleName %in% c("high_lps_1_6hr", "low_lps_9_24hr"))]

plotPCA(bcb_sub)
plotCorrelationHeatmap(bcb_sub)
saveData(bcb_sub, dir="data")
```

Clustering looks really promising.
