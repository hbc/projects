---
title: "Differential Expression - low LPS"
...
Data was subset to only the low LPS samples and any samples without paired time points removed.
...
deDir <- file.path(params$outputDir, "results", "differential_expression", "lowlps")

# Load bcbioRNASeq object
bcbName <- load(params$bcbFile)
bcb <- get(bcbName, inherits = FALSE)
bcb <- bcb[,which(colData(bcb)$lpsLevel=="low")]
...
There appears to be a very strong signal, but for consistency with the high LPS DE analysis, I picked an adjusted p-value cutoff of 0.05.
...
res <- resShrunken
res_lowLPS <- res
saveData(res_lowLPS, dir = dataDir)
```
