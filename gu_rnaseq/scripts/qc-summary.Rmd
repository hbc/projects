...
```{r qc-setup}
basicConfig()
project_summary = "/Volumes/orch/group_dir/gu_rnaseq/project/final/2016-06-15_project/project-summary.csv"
counts_file = "/Volumes/orch/group_dir/gu_rnaseq/project/final/2016-06-15_project/combined.counts"
tx2genes_file = "/Volumes/orch/group_dir/gu_rnaseq/project/final/2016-06-15_project/tx2gene.csv"
...
dd <- row2colnames(dd, "sampleid")
ggplot(dd, aes(x=Mapped, y=Genes.Detected, color=sampleid)) +
    geom_point(size=4) +
    theme_bw(base_size=10) +
    theme(panel.grid.major = element_line(size = .5, color = "grey"),
          axis.text.x = element_text(angle=90)) +
...
Definitely seeing an lower gene detection rate and a lower rRNA rate with the new samples. 
...
Samples are clustering by batch, but the new samples don't appear to be as different from the previous good lung samples as the old bad lung samples were.
