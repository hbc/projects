...
```{r diffexp-functions}
write_diffexp_table = function(df, name, direction) {
	out_file = paste(name, "_", direction, ".txt", sep="")
...
process_multiple_hits = function(df, field="id") {
...
process_regulation = function(df, pair, name) {
	...
	write_diffexp_table(down, name, "downregulated")
	wrap_gvistable(down)
	up = process_upregulated(df, pair, name)
	write_diffexp_table(up, name, "upregulated")
	wrap_gvistable(up)
	return(c(down, up))
}
```
