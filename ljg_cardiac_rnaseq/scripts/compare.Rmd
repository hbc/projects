...
qrtpcr.data <- read.xlsx(qRTPCR.file, sheetIndex = 1)
```
...
## Subset both sets of results to common genes, sort by gene symbol

```{r rawcounts, results='asis'}
names(qrtpcr.data)[2] <- "symbol"

qrtpcr.data <- qrtpcr.data[order(qrtpcr.data$symbol),]
deseq.data <- deseq.data[order(deseq.data$mgi_symbo),]
