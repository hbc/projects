...
##############################################################
# ROC
# Given a gene set of true positives use ROC to evaluate how well the ranked list of gene scores identifies them
# Input: a gene set (list of identifers)
# Input: a data frame of all genes tested. Genes should be listed by the same identifier used in gene set and 
#        minimum of one column required specifying a gene score. Genes should be ordered with best gene score at top. 
# Input: a number indicating which column of dataset contains the score value
##############################################################
rocRanked <- function(geneSet, dataSet, plot=TRUE){
  
  sorted<-dataSet
  remove<-which(geneSet %in% row.names(sorted) == FALSE)
  
  n.genes<-length(row.names(sorted))
  n.validate<-length(geneSet)-length(remove)
  predict<-numeric()
  
  for (i in 1:n.genes){
    
    if(row.names(sorted)[i] %in% geneSet == TRUE){
      val<-1
    }else {val<-0}
    predict<-c(predict, val)
  }
  
  n.vector<-length(predict)
  xVec<-0
  yVec<-0
  x<-0
  y<-0
  
  # Ranking method: incrementing each axis for each of the genes in the ranked list
  for (i in 1:n.vector){
  ...
  if (plot) 
    plot(xVec, yVec, xlab="FP", ylab="TP", pch=".", type="s", main="ROC evaluation")
  
  # Calculating AUC
  top<-sum(yVec)
  bottom<-(n.genes-length(remove))*(n.validate)
  AUC<-top/bottom
  return(AUC)
}
```
...
## Kiebler Comparisons
 - are the genes pulled down in the Kielber et al. publication differnt than those seen here?
...
# get set of genes assayed by both microarray types as mouse ensemblids
## pull in all genes assayed by rat gene 1.0 ST array (used for Kiebler et al.) as rat ensembl ids
## note that we assume they did not filter out any genes/probes as there is no evidence of it in their publication
library("ragene10sttranscriptcluster.db")
assayed.kiebler.rat.ensemblids <- keys(ragene10sttranscriptcluster.db, "ENSEMBL")
## translate these rat ensemblids to their mouse ensemblid homologs using biomart
assayed.kiebler.mus.ensemblids <- getBM(assayed.kiebler.rat.ensemblids, filters="ensembl_gene_id", attributes="mmusculus_homolog_ensembl_gene", mart=ensemblmart.rat) %>% unlist(.) %>% as.vector(.)

# pull in all genes assayed by mogene arrays (after filtering in this experiment)
assayed.renal.mus.ensemblids <- select(mogene10sttranscriptcluster.db, keys = as.character(all.results[[1]]$stats.eset$probesetid), columns = "ENSEMBL", keytype = "PROBEID") %>% .$ENSEMBL

# find intersection of the ensemblids that form the common background of both experiments
common.assayed.ensemblids <- intersect(assayed.renal.mus.ensemblids, assayed.kiebler.mus.ensemblids)
num.common.assayed.ensemblids <- length(common.assayed.ensemblids)

# Now we can limit our pulldown genes from each dataset to this common set of genes
# pull in kiebler data, translate rgd symbols to rat ensembl ids and then find mouse homolog ensembl ids
kiebler.data <- read.csv(file.path(dataDir, "mmc2.csv"))
kiebler.rat.symbols <- unique(kiebler.data$Symbol)
kiebler.rat.ensemblids <- getBM(kiebler.rat.symbols, filters="rgd_symbol", attributes="ensembl_gene_id", mart=ensemblmart.rat)
kiebler.mus.ensemblids <- getBM(kiebler.rat.ensemblids, filters="ensembl_gene_id", attributes=c("mmusculus_homolog_ensembl_gene"), mart=ensemblmart.rat) %>% unlist(.) %>% as.vector(.)
num.kiebler.mus.ensemblids <- length(kiebler.mus.ensemblids)
...
renal.ensemblids <- select(mogene10sttranscriptcluster.db, keys = as.character(renal.data$probesetid), columns = "ENSEMBL", keytype = "PROBEID") %>% .$ENSEMBL
num.renal.ensemblids <- length(renal.ensemblids)


# find the intersection of enriched ensembl ids
overlap.ensemblids <- intersect(kiebler.mus.ensemblids, renal.ensemblids )
num.overlap <- length(overlap.ensemblids)

plot(venneuler(c(Neural=num.kiebler.mus.ensemblids, Renal=num.renal.ensemblids, "Neural&Renal"=num.overlap)))

pval.overlap <-1- phyper(num.overlap, num.renal.ensemblids, num.common.assayed.ensemblids, num.kiebler.mus.ensemblids)


```

Of the total `r num.common.assayed.ensemblids` homologs assayed by both experiments, the Kiebler data showed enrichment of `r num.kiebler.mus.ensemblids` homologs at a log2 fold change of at least `r lfc.cutoff` and an adjuste p-value of less than `r pvalue.cutoff` and the renal cells analysed here, `r num.renal.ensemblids` homologs. Of these enriched homologs, only `r num.overlap` were enriched in both assays, a non-significant overlap  (pvalue = `r pval.overlap`) as determined by a hypergeometric test i.e. an overlap this small could be expected to occur at random.

The genes from both experiments are:

```{r overlapgenes, results='asis'}
overlap.symbols <- getBM(overlap.ensemblids, filters="ensembl_gene_id", attributes="mgi_symbol", mart=ensemblmart.mus) %>% unlist(.) %>% as.vector(.)
kable(overlap.symbols)
```
