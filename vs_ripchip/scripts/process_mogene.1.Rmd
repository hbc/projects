...
top.results.Stau2 <- top.results.Stau2 %>% tbl_df() 

eset.of.interest <- top.results.Stau2 %>% select(.,   contains("symbol"), contains("whole"), logFC)
names(eset.of.interest) <- gsub("_whole", "", names(eset.of.interest))
eset.of.interest <- arrange(eset.of.interest, desc(logFC) ) %>% select(., -logFC, -`wt1-1`)
...
# annotations
pd <- pData(affyNorm.core) %>% tbl_df() %>% filter(., pulldown!="Wt1")
