...
vcplot(stats=all.results[[3]]$stats.eset, plottitle="WT1 whole RNA pulldown vs. input" , lfc.cutoff = lfc.cutoff, pval.cutoff = pvalue.cutoff, shade.colour=highlight.color, axistitlefontsize = 28, axislabelfontsize = 18)          
```
...
top.uniq.results.Stau2 <- top.uniq.results.Stau2 %>% tbl_df()

eset.of.interest <- top.uniq.results.Stau2 %>% select(.,  contains("symbol"), contains("whole"), logFC)
names(eset.of.interest) <- gsub("_whole", "", names(eset.of.interest))
eset.of.interest <- arrange(eset.of.interest, desc(logFC) ) %>% select(., -logFC)

# calculate fold changes from average input intensity
eset.of.interest <- mutate(eset.of.interest, inputavg=select(eset.of.interest, contains("input")) %>% rowMeans )
fcs <-  select(eset.of.interest, -inputavg, -symbol) %>% mutate_each(., funs(ratio=./eset.of.interest$inputavg))


# annotations
pd <- pData(affyNorm.core) %>% tbl_df()
heatmap.annots <- pd %>% filter(., source=="whole_RNA") %>% mutate(., sampleID=sub("_whole", "", sampleID)) %>% select(., sampleID, pulldown)
...
pheatmap(select(eset.of.interest, -symbol, -inputavg), 
         annotation=heatmap.annots, 
         color = colorRampPalette(rev(brewer.pal(n = 11, name = "RdBu")))(100),
         #color=rev(brewer.pal(11,"RdBu")), 
         cluster_cols = FALSE, 
         cluster_rows = TRUE,
         main="Genes enriched after pulldown with Stau2, clustered by row", 
         fontsize=12,
         fontsize_row=7,
         annotation_colors=ann_colors,
         show_colnames=TRUE, 
         show_rownames=FALSE,
        cellwidth=40, annotation_names_col = FALSE, scale="none")
...
pheatmap(select(eset.of.interest, -symbol, -inputavg), 
         ...
         cluster_cols = FALSE, 
         cluster_rows = FALSE,
         main="Genes enriched after pulldown with Stau2, sorted by logFC of Stau2 vs. input (highest on top)", 
         fontsize=12,
         fontsize_row=7,
         annotation_colors=ann_colors,
         show_colnames=TRUE, 
         show_rownames=FALSE,
        cellwidth=40, annotation_names_col = FALSE, scale="none", sub="Fas")

pheatmap(as.matrix(select(eset.of.interest, -symbol,-inputavg)), 
         annotation=heatmap.annots, 
         color = colorRampPalette(rev(brewer.pal(n = 11, name = "RdBu")))(100),
         #color=rev(brewer.pal(11,"RdBu")), 
         cluster_cols = FALSE, 
...
pheatmap(as.matrix(select(eset.of.interest, -symbol)), 
         annotation=heatmap.annots, 
         color = colorRampPalette(rev(brewer.pal(n = 11, name = "RdBu")))(100),
         #color=rev(brewer.pal(11,"RdBu")), 
         cluster_cols = FALSE, 
...
top.uniq.results.Stau2 <- top.uniq.results.Stau2 %>% tbl_df()
top.uniq.results.WT1 <- top.uniq.results.WT1 %>% tbl_df()


# grab top 50 genes by logFC
eset.of.interest.Stau2 <- top.uniq.results.Stau2 %>%  
  arrange(., desc(logFC) ) %>% 
  select(.,  contains("symbol"), contains("whole"), logFC) %>%
  .[1:numtop,]
names(eset.of.interest.Stau2) <- gsub("_whole", "", names(eset.of.interest.Stau2))
eset.of.interest.Stau2 <- mutate(eset.of.interest.Stau2, pulldown="Stau2")
# cluster with hclust and reorder matrix
eset.of.interest.Stau2 <- select(eset.of.interest.Stau2, -logFC, -symbol, -pulldown) %>% 
  as.matrix(.) %>% 
...
eset.of.interest.WT1 <- top.uniq.results.WT1 %>%  
  arrange(., logFC ) %>% 
  select(.,  contains("symbol"), contains("whole"), logFC) %>%
  .[(nrow(.)-numtop):nrow(.),]
names(eset.of.interest.WT1) <- gsub("_whole", "", names(eset.of.interest.WT1))
eset.of.interest.WT1 <- mutate(eset.of.interest.WT1, pulldown="WT1")
# cluster with hclust and reorder matrix
eset.of.interest.WT1 <- select(eset.of.interest.WT1, -logFC, -symbol, -pulldown) %>% 
  as.matrix(.) %>% 
  dist(., method="euclidean") %>% 
  hclust(., method="centroid") %>%
  .$order %>%
  eset.of.interest.WT1[.,]



eset.of.interest <- dplyr::bind_rows(eset.of.interest.Stau2, eset.of.interest.WT1)
# annotations
pd <- pData(affyNorm.core) %>% tbl_df()
...
pheatmap(as.matrix(select(eset.of.interest, -symbol, -logFC, -pulldown)),
         annotation=heatmap.annots, 
         color = colorRampPalette(rev(brewer.pal(n = 11, name = "RdBu")))(100),
         #color=rev(brewer.pal(11,"RdBu")), 
         cluster_cols = FALSE, 
         cluster_rows = FALSE,
         main="Genes enriched after pulldown with Stau2 or WT1", 
         fontsize=12,
         fontsize_row=8,
         annotation_colors=ann_colors,
         show_colnames=TRUE, 
         show_rownames=TRUE,
         labels_row = eset.of.interest$symbol,
...
renal.mus.ensemblids <- AnnotationDbi::select(mogene10sttranscriptcluster.db, keys = as.character(renal.data$probesetid), columns = "ENSEMBL", keytype = "PROBEID") %>% .$ENSEMBL %>% unique(.) %>% na.omit(.)
...
kusek.data <- kusek.data[kusek.data$ensembl_gene_id %in% overlap.probeids,]
kiebler.data <- kiebler.data[kiebler.data$ensembl_gene_id %in% overlap.probeids,]

overlap.renal.data <- merge(merge(renal.data, kiebler.data, by="ensembl_gene_id", all=TRUE), kusek.data, by="ensembl_gene_id", all=TRUE) %>% tbl_df()
...
overlap.renal.data <- arrange(overlap.renal.data, enriched_in_neural_study, symbol)

library(xlsx)


write.xlsx(overlap.renal.data, file=file.path(resultsDir, "Probe.data.for.genes.also.enriched.in.Kusek.and.Kiebler.studies.xlsx"), sheetName="Sheet1", showNA=FALSE)
