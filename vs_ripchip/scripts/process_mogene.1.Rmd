...
# Maher data
maher.data <- read.csv(file.path(dataDir, "Maher-Table S1.csv"),na.strings = c("", " "))
maher.rat.symbols <-maher.data$symbol %>% unique(.) %>% na.omit(.)
maher.mus.ensemblids <- as.vector(unlist(getBM(maher.rat.symbols, filters="rgd_symbol", attributes="mmusculus_homolog_ensembl_gene", mart=ensemblmart.rat)))
write(maher.mus.ensemblids,file.path(resultsDir, "maher.mus.ensemblids.txt"))

# Furic data
# take union of genes pulled down by both isoforms
furic59.data <- read.csv(file.path(dataDir, "Furic-TableS2-Stau2_59.csv"),na.strings = c("", " "))
furic59.hsap.symbols <- na.omit(unique(furic59.data$symbol))
furic62.data <- read.csv(file.path(dataDir, "Furic-TableS3-Stau2_62.csv"))
furic62.hsap.symbols <- na.omit(unique(furic62.data$symbol))
furic.hsap.symbols <- union(furic59.hsap.symbols, furic62.hsap.symbols)
# convert human gene symbols to mouse homologs
furic.mus.ensemblids <- as.vector(unlist(getBM(furic.hsap.symbols, filters="hgnc_symbol", attributes="mmusculus_homolog_ensembl_gene", mart=ensemblmart.hsap)))
write(furic.mus.ensemblids,file.path(resultsDir, "furic.mus.ensemblids.txt"))
...
# all reported neural genes
neural.mus.ensemblids <- unique(union(union(maher.mus.ensemblids, furic.mus.ensemblids), kiebler.mus.ensemblids))
write(neural.mus.ensemblids,file.path(resultsDir, "neural.mus.ensemblids.txt"))

png(file.path(resultsDir, "venneuler-2way.png"))
plot(venneuler(c(renal=length(renal.mus.ensemblids), neural=length(neural.mus.ensemblids), "renal&neural"=length(intersect(kiebler.mus.ensemblids, renal.mus.ensemblids)))))
dev.off()


write(intersect(neural.mus.ensemblids, renal.mus.ensemblids), file=file.path(resultsDir, "neural-renal-enriched-genes.txt"))
