...
```{r DESeq-tables.yng.ctla4, echo=FALSE}
...
```{r funct.clusterprofiler_gsea_go_output.yng.ctla4, results="hide", dependson=outputme}
...
```{r funct.clusterprofiler_gsea_kegg.yng.ctla4}
...
```{r funct.clusterprofiler_gsea_kegg_output.yng.ctla4, results="hide", dependson=outputme}
...
Results from the analysis are show below and can be downloaded from the [Downloads](#downloads) section.
 
```{r funct.clusterprofiler_gsea_immune.yng.ctla4}
# pulled down 
...
if (nrow(as.data.frame(egmt))==0){
    cat("**There were no significant results**")
    outputme <- FALSE
} else {
sanitize_datatable(as.data.frame(egmt))
    outputme <- TRUE
...
```{r funct.clusterprofiler_gsea_immune_output.yng.ctla4, results="hide", dependson=outputme}
...
```{r DESeq-tables.yng.pdl1, ref.label="DESeq-tables.yng.ctla4", echo=FALSE}
```
...
```{r funct.clusterprofiler_gsea_go.yng.pdl1, ref.label="funct.clusterprofiler_gsea_go.yng.ctla4"}
```

```{r funct.clusterprofiler_gsea_go_output.yng.pdl1, ref.label="funct.clusterprofiler_gsea_go_output.yng.ctla4", dependson=outputme, results="hide"}
```

###### KEGG Pathway GSEA with clusterProfiler
Analysis was performed to identify any KEGG pathways (with at least `r minGSS` genes in them) that were significantly dysregulated (genes in pathway could be up- or down-regulated simultaneously). The significantly dysregulated pathways (padj. < `r pvalcutoff`) are shown below and can be downloaded from a link in the [Downloads](@downloads) section.

```{r funct.clusterprofiler_gsea_kegg.yng.pdl1, ref.label="funct.clusterprofiler_gsea_kegg.yng.ctla4"}
```

```{r funct.clusterprofiler_gsea_kegg_output.yng.pdl1, ref.label="funct.clusterprofiler_gsea_kegg_output.yng.ctla4", dependson=outputme, results="hide"}
```
...
```{r funct.clusterprofiler_gsea_immune.yng.pdl1, ref.label="funct.clusterprofiler_gsea_immune.yng.ctla4"}
```

```{r funct.clusterprofiler_gsea_immune_output.yng.pdl1, ref.labe="funct.clusterprofiler_gsea_immune_output.yng.ctla4", dependson=outputme, results="hide"}
```
...
```{r funct.clusterprofiler_gsea_go.old.ctla4, ref.label="funct.clusterprofiler_gsea_go.yng.ctla4"}

```

```{r funct.clusterprofiler_gsea_go_output.old.ctla4, ref.label="funct.clusterprofiler_gsea_go_output.yng.ctla4", dependson=outputme, results="hide"}
...
```{r funct.clusterprofiler_gsea_kegg.old.ctla4, ref.label="funct.clusterprofiler_gsea_kegg.yng.ctla4"}
```

```{r funct.clusterprofiler_gsea_kegg_output.old.ctla4, ref.label="funct.clusterprofiler_gsea_kegg_output.yng.ctla4", dependson=outputme, results="hide"}
```
...
```{r funct.clusterprofiler_gsea_immune.old.ctla4, ref.label="funct.clusterprofiler_gsea_immune.yng.ctla4"}

```

```{r funct.clusterprofiler_gsea_immune_output.old.ctla4, ref.labe="funct.clusterprofiler_gsea_immune_output.yng.ctla4", dependson=outputme, results="hide"}
```
...
```{r funct.clusterprofiler_gsea_go.old.pdl1, ref.label="funct.clusterprofiler_gsea_go.yng.ctla4"}
```

```{r funct.clusterprofiler_gsea_go_output.old.pdl1, ref.label="funct.clusterprofiler_gsea_go_output.yng.ctla4", dependson=outputme, results="hide"}
```

###### KEGG Pathway GSEA with clusterProfiler
Analysis was performed to identify any KEGG pathways (with at least `r minGSS` genes in them) that were significantly dysregulated (genes in pathway could be up- or down-regulated simultaneously). The significantly dysregulated pathways (padj. < `r pvalcutoff`) are shown below and can be downloaded from a link in the [Downloads](@downloads) section.

```{r funct.clusterprofiler_gsea_kegg.old.pdl1, ref.label="funct.clusterprofiler_gsea_kegg.yng.ctla4"}
```

```{r funct.clusterprofiler_gsea_kegg_output.old.pdl1, ref.label="funct.clusterprofiler_gsea_kegg_output.yng.ctla4", dependson=outputme, results="hide"}
```
...
```{r funct.clusterprofiler_gsea_immune.old.pdl1, ref.label="funct.clusterprofiler_gsea_immune.yng.ctla4"}
```

```{r funct.clusterprofiler_gsea_immune_output.old.pdl1, ref.labe="funct.clusterprofiler_gsea_immune_output.yng.ctla4", dependson=outputme, results="hide"}
```
...
```{r DESeq-tables.iso, ref.label="DESeq-tables.yng.ctla4", echo=FALSE}
```
...
```{r funct.clusterprofiler_gsea_go_output.iso, results="hide", dependson=outputme}
```

###### KEGG Pathway GSEA with clusterProfiler
Analysis was performed to identify any KEGG pathways (with at least `r minGSS` genes in them) that were significantly dysregulated (genes in pathway could be up- or down-regulated simultaneously). The significantly dysregulated pathways (padj. < `r pvalcutoff`) are shown below and can be downloaded from a link in the [Downloads](@downloads) section.

```{r funct.clusterprofiler_gsea_kegg.iso, ref.label="funct.clusterprofiler_gsea_kegg.yng.ctla4"}
```

```{r funct.clusterprofiler_gsea_immune.iso, ref.label="funct.clusterprofiler_gsea_kegg_output.yng.ctla4", results="hide", dependson=outputme}
```
...
```{r funct.clusterprofiler_gsea_immune.iso, ref.label="funct.clusterprofiler_gsea_immune.yng.ctla4"}
```


```{r funct.clusterprofiler_gsea_immune_output.iso, ref.label="funct.clusterprofiler_gsea_immune_output.yng.ctla4", results="hide", dependson=outputme}
```
...
```{r DESeq-tables.diff.ctla4, echo=FALSE}
out_df = as.data.frame(res)
out_df$id = rownames(out_df)
out_df = out_df[, c("id", colnames(out_df)[colnames(out_df) != "id"])]
...
```{r funct.gprofiler_revigo.diff.ctla4, ref.label="funct.gprofiler_revigo.yng.ctla4"}
```
...
```{r funct.clusterprofiler_gsea_go_output.diff.ctla4, dependson=outputme, results="hide"}
tempoutfile <-file.path(tempdir(), paste("clusterprofiler_gsea_GO_results_for", contrast_name, "xlsx", sep="."))
rio::export(gseaGOresults, file=tempoutfile)
drop_upload(file=tempoutfile, dest=dropboxfiledir, overwrite=TRUE)
assign(paste("clusterprofiler_gsea_GO_results_for_", contrast_id, sep=""), drop_share(file.path(dropboxfiledir, paste("clusterprofiler_gsea_GO_results_for", contrast_name, "xlsx", sep=".")))$url)
```

###### KEGG Pathway GSEA with clusterProfiler
Analysis was performed to identify any KEGG pathways (with at least `r minGSS` genes in them) that were significantly dysregulated (genes in pathway could be up- or down-regulated simultaneously). The significantly dysregulated pathways (padj. < `r pvalcutoff`) are shown below and can be downloaded from a link in the [Downloads](@downloads) section.

```{r  funct.clusterprofiler_gsea_kegg.diff.ctla4, ref.label="funct.clusterprofiler_gsea_kegg.diff.ctla4"}
```

```{r funct.clusterprofiler_gsea_kegg_output.diff.ctla4, dependson=outputme,  results="hide"}
tempoutfile <-file.path(tempdir(), paste("clusterprofiler_gsea_kegg_results_for", contrast_name, "xlsx", sep="."))
rio::export(kkgsea_results, file=tempoutfile)
drop_upload(file=tempoutfile, dest=dropboxfiledir, overwrite=TRUE)
assign(paste("clusterprofiler_gsea_kegg_results_for_", contrast_id, sep=""), drop_share(file.path(dropboxfiledir, paste("clusterprofiler_gsea_kegg_results_for", contrast_name, "xlsx", sep=".")))$url)
```
...
```{r funct.clusterprofiler_gsea_immune.diff.ctla4, ref.label="funct.clusterprofiler_gsea_immune.yng.ctla4"}
```


```{r funct.clusterprofiler_gsea_immune_output.diff.ctla4, dependson=outputme, results="hide"}
tempoutfile <-file.path(tempdir(), paste("clusterprofiler_gsea_immune_results_for", contrast_name, "xlsx", sep="."))
rio::export(as.data.frame(egmt), file=tempoutfile)
drop_upload(file=tempoutfile, dest=dropboxfiledir, overwrite=TRUE)
assign(paste("clusterprofiler_gsea_immune_results_for_", contrast_id, sep=""), drop_share(file.path(dropboxfiledir, paste("clusterprofiler_gsea_immune_results_for", contrast_name, "xlsx", sep=".")))$url)
```
...
```{r DESeq-tables.diff.pdl1, ref.label="DESeq-tables.diff.ctla4", echo=FALSE}
```
...
```{r  funct.clusterprofiler_gsea_go.diff.pdl1, ref.label="funct.clusterprofiler_gsea_go.yng.ctla4"}
```

```{r funct.clusterprofiler_gsea_go_output.diff.pdl1, ref.label="funct.clusterprofiler_gsea_go_output.diff.ctla4", dependson=outputme, results="hide"}
```

###### KEGG Pathway GSEA with clusterProfiler
Analysis was performed to identify any KEGG pathways (with at least `r minGSS` genes in them) that were significantly dysregulated (genes in pathway could be up- or down-regulated simultaneously). The significantly dysregulated pathways (padj. < `r pvalcutoff`) are shown below and can be downloaded from a link in the [Downloads](@downloads) section.

```{r  funct.clusterprofiler_gsea_kegg.diff.pdl1, ref.label="funct.clusterprofiler_gsea_kegg.yng.ctla4"}
```

```{r funct.clusterprofiler_gsea_kegg_output.diff.pdl1, ref.label="funct.clusterprofiler_gsea_kegg_output.diff.ctla4", dependson=outputme, results="hide"}
```
...
```{r funct.clusterprofiler_gsea_immune_output.diff.pdl1, ref.label="funct.clusterprofiler_gsea_immune_output.diff.ctla4", dependson=outputme, results="hide"}
...
```{r eval=exists("clusterprofiler_gsea_GO_results_for_Young_vs_Old_of_iso_treated_mice"), results="asis", echo=FALSE }
cat(paste("[clusterprofiler GSEA GO results for Young versus Old in Isotype treated mice](", clusterprofiler_gsea_GO_results_for_Young_vs_Old_of_iso_treated_mice, ")", sep=""))
```

```{r eval=exists("clusterprofiler_gsea_GO_results_for_CTLA4_old_and_young_difference_of_differences"), results="asis", echo=FALSE }
cat(paste("[clusterprofiler GSEA GO results for CTLA4 versus Isotype differences in how Old and young mice respond](", clusterprofiler_gsea_GO_results_for_CTLA4_old_and_young_difference_of_differences, ")", sep=""))
```

```{r eval=exists("clusterprofiler_gsea_GO_results_for_PDL1_old_and_young_difference_of_differences"), results="asis", echo=FALSE  }
cat(paste("[clusterprofiler GSEA GO results for PDL1 versus Isotype differences in how Old and young mice respond](", clusterprofiler_gsea_GO_results_for_PDL1_old_and_young_difference_of_differences, ")", sep=""))
```
...
```{r, eval=exists("clusterprofiler_gsea_kegg_results_for_ctla4_vs_iso_of_Young_mice"), results="asis", echo=FALSE  }
cat(paste("[clusterProfiler GSEA KEGG results for CTLA4 versus Isotype in Young Mice](", clusterprofiler_gsea_kegg_results_for_ctla4_vs_iso_of_Young_mice, ")", sep=""))
```

```{r, eval=exists("clusterprofiler_gsea_kegg_results_for_pdl1_vs_iso_of_Young_mice"), results="asis", echo=FALSE  }
cat(paste("[clusterProfiler GSEA KEGG results for PDL1 versus Isotype in Young Mice](", clusterprofiler_gsea_kegg_results_for_pdl1_vs_iso_of_Young_mice, ")", sep=""))
```

```{r eval=exists("clusterprofiler_gsea_kegg_results_for_ctla4_vs_iso_of_Old_mice"), results="asis", echo=FALSE  }
cat(paste("[clusterProfiler GSEA KEGG results for CTLA4 versus Isotype in Old Mice](", clusterprofiler_gsea_kegg_results_for_ctla4_vs_iso_of_Old_mice, ")", sep=""))
```

```{r eval=exists("clusterprofiler_gsea_kegg_results_for_pdl1_vs_iso_of_Old_mice"), results="asis" , echo=FALSE }
 cat(paste("[clusterProfiler GSEA KEGG results for PLD1 versus Isotype in Old Mice](", clusterprofiler_gsea_kegg_results_for_pdl1_vs_iso_of_Old_mice, ")", sep=""))
```

```{r eval=exists("clusterprofiler_gsea_kegg_results_for_Young_vs_Old_of_iso_treated_mice"), results="asis" , echo=FALSE }
cat(paste("[clusterProfiler GSEA KEGG results for Young versus Old in Isotype treated mice](", clusterprofiler_gsea_kegg_results_for_Young_vs_Old_of_iso_treated_mice, ")", sep=""))
```

```{r eval=exists("clusterprofiler_gsea_kegg_results_for_CTLA4_old_and_young_difference_of_differences"), results="asis", echo=FALSE  }
cat(paste("[clusterprofiler GSEA KEGG results for CTLA4 versus Isotype differences in how Old and young mice respond](", clusterprofiler_gsea_kegg_results_for_CTLA4_old_and_young_difference_of_differences, ")", sep=""))
```

```{r eval=exists("clusterprofiler_gsea_kegg_results_for_PDL1_old_and_young_difference_of_differences"), results="asis" , echo=FALSE }
cat(paste("[clusterprofiler GSEA KEGG results for PDL1 versus Isotype differences in how Old and young mice respond](", clusterprofiler_gsea_kegg_results_for_PDL1_old_and_young_difference_of_differences, ")", sep=""))
```
...
```{r, eval=exists("clusterprofiler_gsea_immune_results_for_ctla4_vs_iso_of_Young_mice"), results="asis" , echo=FALSE }
cat(paste("[clusterProfiler GSEA Immune results for CTLA4 versus Isotype in Young Mice](", clusterprofiler_gsea_immune_results_for_ctla4_vs_iso_of_Young_mice, ")", sep=""))
```

```{r, eval=exists("clusterprofiler_gsea_immune_results_for_pdl1_vs_iso_of_Young_mice"), results="asis" , echo=FALSE }
cat(paste("[clusterProfiler GSEA Immune results for PDL1 versus Isotype in Young Mice](", clusterprofiler_gsea_immune_results_for_pdl1_vs_iso_of_Young_mice, ")", sep=""))
```

```{r eval=exists("clusterprofiler_gsea_immune_results_for_ctla4_vs_iso_of_Old_mice"), results="asis", echo=FALSE  }
cat(paste("[clusterProfiler GSEA Immune results for CTLA4 versus Isotype in Old Mice](", clusterprofiler_gsea_immune_results_for_ctla4_vs_iso_of_Old_mice, ")", sep=""))
```

```{r eval=exists("clusterprofiler_gsea_immune_results_for_pdl1_vs_iso_of_Old_mice"), results="asis" , echo=FALSE }
cat(paste("[clusterProfiler GSEA Immune results for PLD1 versus Isotype in Old Mice](", clusterprofiler_gsea_immune_results_for_pdl1_vs_iso_of_Old_mice, ")", sep=""))
```

```{r eval=exists("clusterprofiler_gsea_immune_results_for_Young_vs_Old_of_iso_treated_mice"), results="asis" , echo=FALSE }
cat(paste("[clusterProfiler GSEA Immune results for Young versus Old of Isotype treated mice](", clusterprofiler_gsea_immune_results_for_Young_vs_Old_of_iso_treated_mice, ")", sep=""))
```

```{r eval=exists("clusterprofiler_gsea_immune_results_for_CTLA4_old_and_young_difference_of_differences"), results="asis", echo=FALSE  }
cat(paste("[clusterprofiler GSEA Immune results for CTLA4 versus Isotype differences in how Old and young mice respond](", clusterprofiler_gsea_immune_results_for_CTLA4_old_and_young_difference_of_differences, ")", sep=""))
```

```{r eval=exists("clusterprofiler_gsea_immune_results_for_PDL1_old_and_young_difference_of_differences"), results="asis", echo=FALSE }
cat(paste("[clusterprofiler GSEA Immune results for PDL1 versus Isotype differences in how Old and young mice respond](", clusterprofiler_gsea_immune_results_for_PDL1_old_and_young_difference_of_differences, ")", sep=""))
```
