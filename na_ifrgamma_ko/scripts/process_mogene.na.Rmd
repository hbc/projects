...
datatable(DCsig.annot, caption = "Updated signature with human and mouse homology gene annotations")

IFNGsig <- read_csv(file.path(dataDir, "Results_GSE18686_IFNGsignature_DEG_byMSF.csv"))
# drop probes without EntrezIDs
IFNsig <- dplyr::select(IFNGsig, SYMBOL, lgFCH_TrtIFNgIFNG, ENTREZID)
```    


For both hypergeometric overlap and GSEA-style analyses, will use the two gene signatures as lists of genes without rank. Will try splitting the IFNG genes into those up/down regulated as well to see if anything interesting comes up.

Will perform these analyses using the clusterProfiler `r citep("10.1089/omi.2011.0118")` package from Bioconductor, which lets you use custom gene sets to perform both hypergeometric and GSEA analyses of gene expression data. To do so, I have first have to annotate which genes analyzed in the Ifngr1 KO analysis are part of which gene signature. Used EntrezIDs to match up gene signatures to the Ifngr1 KO analysis, removing any genes/probes without EntrezIDs.

```{r annotatebackgrou}
bkgrndgenes <- all.results[[1]]$stats.eset %>% tbl_df() %>% dplyr::select(., entrezID) %>% mutate(., DCsig=ifelse(entrezID %in% DCsig.annot$mouse_entrezid, 1, 0))
...
filter Ifngr1 KO data for NA values in entrezIDs
