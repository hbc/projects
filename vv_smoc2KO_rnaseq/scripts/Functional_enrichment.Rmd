...
for(pathid in path.ids){
  ...
  entrez_results.sub <- entrez_results.sub.annot[,names(entrez_results.sub.annot) %in% as.character(row.names(metadata))]
  #sanity check
  identical(as.character(names(entrez_results.sub)), as.character(row.names(metadata)))
  # heatmap of normzlied, variant stabilized counts for all gene within seti
  if(nrow(entrez_results.sub)>120){showrownames=FALSE}else{showrownames=TRUE}
  pheatmap(entrez_results.sub, labels_row=entrez_results.sub.annot$mgi_symbol, show_rownames = showrownames,scale="row", fontsize_row=8, annotation=metadata, main=paste("All Genes", pathid, sep=" - "))
  # take the top quartile (by absolute(log2foldchange)) of these genes
  top.entrez_results.sub.annot <- subset(entrez_results.sub.annot, abs(entrez_results.sub.annot$log2FoldChange)>quantile(abs(entrez_results.sub.annot$log2FoldChange),0.75))
  ...
  pheatmap(top.entrez_results.sub, labels_row=top.entrez_results.sub.annot$mgi_symbol, show_rownames = showrownames,scale="row", fontsize_row=8, annotation=metadata, main=paste("Top Quartile Genes", pathid, sep=" - "))
  }  
```  
...
```{r enrichegoheatmaps, fig.height=18}
enriched.gos.bp <- row.names(gores.bp$greater[sel.bp,, drop=FALSE])
enriched.gos.cc <- row.names(gores.cc$greater[sel.cc,, drop=FALSE])
enriched.gos.mf <- row.names(gores.mf$greater[sel.mf,, drop=FALSE])

enriched.gos <- c(enriched.gos.bp, enriched.gos.cc, enriched.gos.mf)
...
for(enrichedgo in enriched.gos){
  ...
  # heatmap of normzlied, variant stabilized counts for all gene within seti
  if(nrow(entrez_results.sub)>120){showrownames=FALSE}else{showrownames=TRUE}
  pheatmap(entrez_results.sub, labels_row=entrez_results.sub.annot$mgi_symbol, show_rownames = showrownames,scale="row", fontsize_row=8, annotation=metadata, main=paste("All Genes", enrichedgo, sep=" - "))
  # take the top quartile (by absolute(log2foldchange)) of these genes
  top.entrez_results.sub.annot <- subset(entrez_results.sub.annot, abs(entrez_results.sub.annot$log2FoldChange)>quantile(abs(entrez_results.sub.annot$log2FoldChange),0.75))
  top.entrez_results.sub <- top.entrez_results.sub.annot[,names(top.entrez_results.sub.annot) %in% row.names(metadata)]
   #sanity check again
  identical(as.character(names(top.entrez_results.sub)), as.character(row.names(metadata)))
    # heatmap of normzlied, variant stabilized counts for the genes with the biggest logFCs with the set
    if(nrow(top.entrez_results.sub)>120){showrownames=FALSE}else{showrownames=TRUE}
  pheatmap(top.entrez_results.sub, labels_row=top.entrez_results.sub.annot$mgi_symbol, show_rownames = showrownames,scale="row", fontsize_row=8, annotation=metadata, main=paste("Top Quartile Genes", enrichedgo, sep=" - "))
}   
```  
