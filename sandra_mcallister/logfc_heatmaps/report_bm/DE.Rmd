---
title: "Differential Expression - Pairwise comparisons"
...
```{r alpha_summary_bothgcsf, results="asis"}
alphaSummary(dds, contrast=c("treatment", "BOTH", "GCSF"))
```

## BOTH versus ZA

```{r alpha_summary_bothza, results="asis"}
alphaSummary(dds, contrast=c("treatment", "BOTH", "ZA"))
```
...
plotVolcano(res_bothgcsf, lfc = lfc,alpha = alpha, ntop=20, histograms=FALSE)
```

#### BOTH versus ZA

```{r plot_volcano_bothza}
plotVolcano(res_bothza, lfc = lfc,alpha = alpha, ntop=20, histograms=FALSE)
```
...
plotDEGHeatmap(res_bothgcsf, counts = rld)
```

#### BOTH versus ZA
```{r plot_deg_heatmap_bothza}
plotDEGHeatmap(res_bothza, counts = rld)
```
...
resTbl_bothgcsf <- resultsTables(res_bothgcsf, lfc = lfc, dir = deDir)
resTbl_bothza <- resultsTables(res_bothza, lfc = lfc, dir = deDir)
```
...
```{r top_tables_bothgcsf, results="asis"}
topTables(resTbl_bothgcsf)
```

### BOTH versus ZA 

```{r top_tables_bothza, results="asis"}
topTables(resTbl_bothza)
```

## Functional Analyses (for all pairwise comparisons)
...
### Using top DE genes from all pairwise comparisons to control {.tabset}
...
pathway_stats_all <- pathway_stats_all[!(apply(dplyr::select(pathway_stats_all, -symbol), 1, function(x) all(is.na(x)))),]
cat("#### row scaled")
cat("**clustered by row")
pheatmap(as.matrix(dplyr::select(pathway_stats_all, -symbol)), scale="row", color = colorRampPalette(rev(brewer.pal(n = 8, name =  "RdBu")))(100), fontsize_row = 8)
cat("#### Not row scaled")
cat("**clustered by row")
pheatmap(as.matrix(dplyr::select(pathway_stats_all, -symbol)), color = colorRampPalette(rev(brewer.pal(n = 8, name =  "RdBu")))(100), fontsize_row = 8)

```

#### Add Gene annotations

```{r annotated_heatmaps, fig.height=16, fig.width=6, results='asis'}

library(rio)
library(janitor)
library(dplyr)
library(tidyr)
gene_categories <- import(file.path("meta", "three_groupings_for_heatmaps.xlsx")) %>% 
 clean_names()
names(gene_categories) <- sub("_differentiation", "", names(gene_categories))
gene_categories <- gather(gene_categories, "category")
gene_categories <- filter(gene_categories, !is.na(value))
row.names(gene_categories) <- gene_categories$value
names(gene_categories) <- c('category', "symbol")

pathway_stats_all <- inner_join(pathway_stats_all, gene_categories, by="symbol")

cbPalette <- c("#999999", "#E69F00", "#56B4E9", "#009E73", "#F0E442", "#0072B2",  "#D55E00", "#CC79A7")
category_colors <- cbPalette[1:3]
names(category_colors) <- unique(unlist(pathway_stats_all$category))
ann_colors = list(category = category_colors)


cat("**clustered by row")

annot_row <- dplyr::select(pathway_stats_all, category, symbol) %>% column_to_rownames("symbol") 
heatmap_data <- dplyr::select(pathway_stats_all, contains("log"), symbol ) %>% column_to_rownames("symbol") %>% as.matrix()
...
cat("**rows sorted by category and then log2FC_ZA**")
pathway_stats_all <- arrange(pathway_stats_all, category, log2FC_ZA)
annot_row <- dplyr::select(pathway_stats_all, category, symbol) %>% column_to_rownames("symbol") 
heatmap_data <- dplyr::select(pathway_stats_all, contains("log"), symbol ) %>% column_to_rownames("symbol") %>% as.matrix()
pheatmap(heatmap_data, 
         color = colorRampPalette(rev(brewer.pal(n = 8, name =  "RdBu")))(100), 
