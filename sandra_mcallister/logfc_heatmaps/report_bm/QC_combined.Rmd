...
Drop BOTH2 (mapping rate), CTL3 and ZA1 (rRNA rate).

```{r dropoutliers}
outlier_samples <- c("CTL3", "ZA1", "BOTH2")
nonoutlier_samples <- which(!(colData(bcb)$sampleName %in% outlier_samples))
bcb_combined_sub <- bcb[,nonoutlier_samples]
saveData(bcb_combined_sub, dir="data")
```
...
plotPCA(bcb_combined_sub)
```

## Covariates correlation with PCs

```{r plot_pca_covariates_sub}
plotPCACovariates(bcb_combined_sub)
```

## Hierarchical clustering

```{r plot_correlation_heatmap_sub, fig.height=8, fig.width=10}
plotCorrelationHeatmap(bcb_combined_sub)
```
