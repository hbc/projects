---
title: "Differential Expression - OCP regression based comparisons with RUVseq batch correction"
...
saveData(res_gcsf, dir = file.path(outputDir, "data", "2018-04-12"))
...
saveData(res_za, dir = file.path(outputDir, "data", "2018-04-12"))
...
saveData(res_gcsfza, dir = file.path(outputDir, "data", "2018-04-12"))
```
...
```{r plot_deg_heatmap_gcsf, fig.height=14}
...
plotDEGHeatmapJH(bcb,res_gcsf, ruv_counts = ruv_counts_temp, cluster_cols=FALSE, gap = 6, title="GCSF Effects - log2FC>1", lfc=1)
```
...
plotDEGHeatmapJH(bcb,res_za, ruv_counts = ruv_counts_temp, cluster_cols=FALSE, gap = 7, title="ZA Effects - log2FC>1", lfc=1)
```

#### GCSF:ZA non-additive effects
```{r plot_deg_heatmap_both, fig.height=16}
...
plotDEGHeatmapJH(bcb,res_gcsfza, ruv_counts = ruv_counts_temp,cluster_cols=FALSE, gap=c(2,6,11), title="GCSF:ZA Non-Additive Effects", lfc=1)
```
...
mean_values <-  ruv_counts[genes_of_interest,]  %>% 
  as.data.frame %>%   
...
pheatmap(mean_values, 
         border_color=NA,  
         #cutree_rows = 8, 
         main="CTL mean centered, range scaled, batch corrected values", 
         show_rownames = FALSE,
         scale="none",
         color = inferno(256))
...
plotGeneHeatmapJH(bcb, ruv_counts = ruv_counts, genes=stats_all_zachange$ensgene)
```


# Pathway visualizations
- viewing differential expression values in context of KEGG pathway for the osteoclast differentiation pathway (hsa04380)

```{r pathwayvis, warning=FALSE, message=FALSE, error=FALSE, results="hide"}
library(pathview)
library(dplyr)

fc_za <- dplyr::filter(tb_za, padj<0.05) %>% dplyr::select(ensgene,log2FoldChange) %>% as.data.frame()
fc_za <- inner_join(fc_za, metadata(bcb)$annotable, by="ensgene") %>% dplyr::select(entrez, log2FoldChange)
fc_za <- fc_za %>% filter(!is.na(entrez)) %>% 
  mutate(entrez=sub("c\\(", "", entrez)) %>% 
  mutate(entrez=gsub("\\)", "", entrez)) %>% 
  separate(entrez,into=c("one","two","three"), sep=", " ) %>% 
  gather(x, entrez, one, two, three) %>% 
  dplyr::select(-x) %>% 
  filter(!is.na(entrez))
fold_changes_za <- setNames( fc_za$log2FoldChange, as.character(fc_za$entrez))

pathview(gene.data = fold_changes_za,
         pathway.id = "04380",
         species = "mouse", 
...
fc_gcsf <- dplyr::filter(tb_gcsf, padj<0.05) %>% dplyr::select(ensgene,log2FoldChange) %>% as.data.frame()
fc_gcsf <- inner_join(fc_gcsf, metadata(bcb)$annotable, by="ensgene") %>% dplyr::select(entrez, log2FoldChange)
fc_gcsf <- fc_gcsf %>% filter(!is.na(entrez)) %>% 
  mutate(entrez=sub("c\\(", "", entrez)) %>% 
  mutate(entrez=gsub("\\)", "", entrez)) %>% 
  separate(entrez,into=c("one","two","three"), sep=", " ) %>% 
  gather(x, entrez, one, two, three) %>% 
  dplyr::select(-x) %>% 
  filter(!is.na(entrez))
fold_changes_gcsf <- setNames( fc_gcsf$log2FoldChange, as.character(fc_gcsf$entrez))

pathview(gene.data = fold_changes_gcsf,
         pathway.id = "04380",
         species = "mouse", 
         limit = list(gene = 2, cpd = 1), 
         out.suffix="gcsf"
...
fc_gcsfza <- dplyr::filter(tb_gcsfza, padj_gcsfza<0.05) %>% dplyr::select(ensgene_gcsfza,log2FoldChange_gcsfza) %>% as.data.frame()
names(fc_gcsfza) <- c("ensgene","log2FoldChange" )
fc_gcsfza <- inner_join(fc_gcsfza, metadata(bcb)$annotable, by="ensgene") %>% dplyr::select(entrez, log2FoldChange)
fc_gcsfza <- fc_gcsfza %>% filter(!is.na(entrez)) %>% 
  mutate(entrez=sub("c\\(", "", entrez)) %>% 
  mutate(entrez=gsub("\\)", "", entrez)) %>% 
  separate(entrez,into=c("one","two","three"), sep=", " ) %>% 
  gather(x, entrez, one, two, three) %>% 
  dplyr::select(-x) %>% 
  filter(!is.na(entrez))
fold_changes_gcsfza <- setNames( fc_gcsfza$log2FoldChange, as.character(fc_gcsfza$entrez))

pathview(gene.data = fold_changes_gcsfza,
         pathway.id = "04380",
         species = "mouse", 
         limit = list(gene = 2, cpd = 1), 
         out.suffix="gcsfza"
...
## ZA effects on osteoclast differentiation pathway

```{r echo=FALSE, out.width='100%'}
knitr::include_graphics('./mmu04380.za.png')
```

## GCSF effects on osteoclast differentiation pathway

```{r echo=FALSE, out.width='100%'}
knitr::include_graphics('./mmu04380.gcsf.png')
```

## GCSF:ZA interaction effects on osteoclast differentiation pathway

```{r echo=FALSE, out.width='100%'}
knitr::include_graphics('./mmu04380.gcsfza.png')
```
