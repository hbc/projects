...
plotDEGHeatmapJH <- function(
  ...
  if (length(genes) < 2) {
    ...
    plotGeneHeatmapJH(bcbioObject,ruv_counts, genes = genes, title = title, ...)
  }
} 
...
plotGeneHeatmapJH <- function(
  ...
  if (isTRUE(showRownames)) {
    annots <- gene2symbol(bcb)
    annots <- annots[row.names(counts),]
   ...
   # counts_annots <- counts_annots %>% as.data.frame() %>% filter(!is.na(padj)) %>% group_by(symbol) %>%  filter(pvalue== min(pvalue)) %>% filter(log2FoldChange==max(log2FoldChange))
    ...
    counts_annots <- as(counts_annots, "DataFrame")
    counts <- counts_annots
  }
