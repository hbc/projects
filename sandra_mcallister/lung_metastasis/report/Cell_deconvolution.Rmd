...
bcbName_immgen_basic <- load(params$bcbFile_immgen_basic)
bcb_immgen_basic <- get(bcbName_immgen_basic)
```

```{r header, child="_header.Rmd", eval=file.exists("_header.Rmd")}
```

```{r subset_bcbs, include=FALSE}
bcb_immgen_basic <- bcb_immgen_basic[,which(colData(bcb_immgen_basic)$sex=="female")]
```
...
tpm_immgen <- tpm(bcb_immgen_basic) %>% as.data.frame()
