...
sig.results <- alply(all.pair.combos, 2, function(combo) {
  ...
  results.1 <- counts(cds.filt, normalize=TRUE)[which(res.filt$padj<pvalcutoff & abs(res.filt$log2FoldChange)>logfoldcutoff),]
  ## get means and pvalues for significant hits and put together with counts
  results.2 <- res.filt[which(res.filt$padj<pvalcutoff & abs(res.filt$log2FoldChange)>logfoldcutoff),]
  ...
  return(list(cond1=combo[1], cond2=combo[2], setname=setname, res.filt=res.filt, results=results,  qqplots=qqplots, DEplots=DEplots))
})  

sig.results.unfilt <- alply(all.pair.combos, 2, function(combo) {
  setname <- paste(combo[1], combo[2], sep="-vs-")
  print(setname)
  ## perform significance testing
...
There are `r nrow(sig.results[[num]]$results)` genes with significant differential expression (FDR<`r pvalcutoff` and a log2 fold change in expression of at least `r logfoldcutoff`)

```{r out1, out.width='50%', fig.show='hold', dev='png'}
sig.results[[num]]$qqplots
sig.results[[num]]$DEplots
```
...
if (nrow(sig.results[[num]]$results)>(numsig-1)) {
  out1 <- sig.results[[num]]$results[1:numsig,]
  } else {
    out1  <- sig.results[[num]]$results
    }
out1.gvis <- gvisTable(as.data.frame(apply(out1, 2, as.character)), options=list(width=1280))
print(out1.gvis, "chart")

write.table(sig.results[[num]]$results, file=file.path(resultsDir, paste("DE.genes.q", pvalcutoff, setnames[num], "xls", sep=".")), quote=F, sep="\t", row.names=F, col.names=T)
```
...
if (nrow(sig.results[[num]]$results)>(numsig-1)) {
  out2 <- sig.results[[num]]$results[1:numsig,]
  } else {
    out2  <- sig.results[[num]]$results
    }
...
if (nrow(sig.results[[num]]$results)>(numsig-1)) {
  out3 <- sig.results[[num]]$results[1:numsig,]
  } else {
    out3  <- sig.results[[num]]$results
    }
...
`r num=1`
[Counts and stats for filtered genes and samples in `r all.pair.combos[1,num]` vs. `r all.pair.combos[2,num]` comparison](`r file.path(resultsDir, paste("all.exprs.stats.genes", setnames[num], "xls", sep="."))` )
...
`r num=1`
[Counts and stats for ALL  genes and samples in `r all.pair.combos[1,num]` vs. `r all.pair.combos[2,num]` comparison](`r file.path(resultsDir, paste("all.exprs.stats.unfiltered.genes", setnames[num], "xls", sep="."))`)
`r num=num+1`
[Counts and stats for ALL genes and samples in `r all.pair.combos[1,num]` vs. `r all.pair.combos[2,num]` comparison](`r file.path(resultsDir, paste("all.exprs.stats.unfiltered.genes", setnames[num], "xls", sep="."))`)
`r num=num+1`
[Counts and stats for ALL genes and samples in `r all.pair.combos[1,num]` vs. `r all.pair.combos[2,num]` comparison](`r file.path(resultsDir, paste("all.exprs.stats.unfiltered.genes", setnames[num], "xls", sep="."))`)
...
sig.results.FAPLung.vs.MEF <- sig.results[[2]]$res.filt[(sig.results[[2]]$res.filt$padj<pvalcutoff & sig.results[[2]]$res.filt$log2FoldChange<logfoldcutoff),]
sig.results.FAPLung.vs.MEF <- sig.results.FAPLung.vs.MEF[grepl("ENS", sig.results.FAPLung.vs.MEF$id),]

sig.results.FAPMuscle.vs.MEF <- sig.results[[3]]$res.filt[(sig.results[[3]]$res.filt$padj<pvalcutoff & sig.results[[3]]$res.filt$log2FoldChange<logfoldcutoff),]
sig.results.FAPMuscle.vs.MEF <- sig.results.FAPMuscle.vs.MEF[grepl("ENS", sig.results.FAPMuscle.vs.MEF$id),]


FAPLung=length(sig.results.FAPLung.vs.MEF$id)
FAPMuscle=length(sig.results.FAPMuscle.vs.MEF$id)
...
grid.newpage()
draw.pairwise.venn(FAPLung, FAPMuscle, num.both.sig,fill = c(cbPalette[2],cbPalette[3]),
                   alpha = c(0.75, 0.75), lwd=0, cat.dist=0,
                   cat.cex=2, cat.fontfamily = 3,
                   cex = 1, fontfamily =3, 
                   category=c("FAPLung", "FAPMuscle"), 
                   euler.d=T, scaled=T)
grid.newpage()
```
...
For the `r sig.results[[idx]]$setname` comparison, there are `r length(unique(sig.results[[1]]$res.filt$id[(sig.results[[idx]]$res.filt$padj<pvalcutoff & sig.results[[idx]]$res.filt$log2FoldChange<logfoldcutoff)]))` unique genes that are differentially expressed, i.e. with at least `r 2^logfoldcutoff` fold change between the `r sig.results[[idx]]$cond1` and `r sig.results[[idx]]$cond2` sample classes with a pvalue of lower than `r pvalcutoff`.

`r idx=idx+1`

For the `r sig.results[[idx]]$setname` comparison, there are `r length(unique(sig.results[[1]]$res.filt$id[(sig.results[[idx]]$res.filt$padj<pvalcutoff & sig.results[[idx]]$res.filt$log2FoldChange<logfoldcutoff)])
)` unique genes that are differentially expressed, i.e. with at least `r 2^logfoldcutoff` fold change between the `r sig.results[[idx]]$cond1` and `r sig.results[[idx]]$cond2` sample classes.
...
prob.overlap  <- 1 - phyper(num.both.sig - 1, FAPLung, totalassayed - FAPLung, FAPMuscle)

## Using OrderLists library
# order significant results by log2foldchange
ID.list.FAPLung <- sig.results[[2]]$res.filt$id[order(sig.results[[2]]$res.filt$log2FoldChange)]
ID.list.FAPMuscle <- sig.results[[3]]$res.filt$id[order(sig.results[[3]]$res.filt$log2FoldChange)]
## compare with OrderedList, let it select alpha
comparedlist <- compareLists(ID.list.FAPLung, ID.list.FAPMuscle, mapping = NULL, two.sided = TRUE, B = 10000, alphas = NULL, min.weight = 1e-5, invar.q = 0.5)
...
`r head(ID.list.FAPLung, rank)`  
The FAPMuscle gene list has:  
`r head(ID.list.FAPMuscle, rank)`
`r overlap=length(intersect(head(ID.list.FAPLung, rank), head(ID.list.FAPMuscle,rank)))`
There are `r length(intersect(head(ID.list.FAPLung, rank), head(ID.list.FAPMuscle,rank)))` genes overlapping between the two lists.  
The weight given to any overlap at this rank is: `r exp(-0.005*rank)`  
The weighted overlap is then: `r exp(-0.005*rank)*(as.numeric(overlap))`.  
...
The a) expression levels and b) differential expression statistics for genes that contribute the most to this overlap are listed below.

```{r overlapgenes, results='asis',eval=FALSE}
overlapgenes <- getOverlap(comparedlist)$intersect

stats.overlapgenes <- merge(sig.results[[1]]$results, sig.results[[2]]$results, by=c('id', 'description', 'mgi_symbol',"FAPLung1", "FAPLung2", "FAPLung3", "FAPLung4", "FAPLung5", "FAPMuscle2", "FAPMuscle3", "FAPMuscle4", "FAPMuscle5", "MEF1", "MEF2", "MEF3", "MEF4"), suffixes=c("_FAPLung.vs.MEF", "_FAPMuscle.vs.MEF"))
