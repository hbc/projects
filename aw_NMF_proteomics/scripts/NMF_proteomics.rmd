...
```{r dataload}
data <- read.xlsx(file.path(dataDir, "michael.xls"), sheetIndex=3)
metadata <- read.xlsx(file.path(metaDir, "metadata.xls"), sheetIndex=1) 

annot <- data[,which(metadata$datatype=="descriptor")]

data.sum <- data[,which(metadata$datatype=="sum")]
data.sum <- log2(data.sum)res
...
Looks promising, though it would be nice to see the BMM and LSC samples clustering together more tightly.
...
It appears that the samples cluster better by type after simple summing of counts, without attempting to normalize. Going forward, worked with the summed counts only.

## NMF

### Estimate factoriation rank

>A critical parameter in NMF is the factorization rank r. It denes the number of metagenes used
to approximate the target matrix. Given a NMF method and the target matrix, a common way of
deciding on r is to try dierent values, compute some quality measure of the results, and choose
...
estim.sum <- nmfEstimateRank(eset.sum, range=2:7, nrun=TRAIN)
plot(estim.sum)
...
estim.sum.rand <- nmf(eset.sum.rand, 2:8, nrun = TRAIN)
...
res.multi.method.3 <- nmf(eset.sum, 3, list("brunet", "KL", "lee", "Frobenius", "offset", "nsNMF", "ls-nmf", "pe-nmf", "snmf/r", "snmf/l", "siNMF"), nrun=TRAIN, seed = 123456, .options = list("t", "v"))
res.multi.method.4 <- nmf(eset.sum, 4, list("brunet", "KL", "lee", "Frobenius", "offset", "nsNMF", "ls-nmf", "pe-nmf", "snmf/r", "snmf/l", "siNMF"), nrun=TRAIN, seed = 123456, .options = list("t", "v"))
res.multi.method.5 <- nmf(eset.sum, 5, list("brunet", "KL", "lee", "Frobenius", "offset", "nsNMF", "ls-nmf", "pe-nmf", "snmf/r", "snmf/l", "siNMF"), nrun=TRAIN, seed = 123456, .options = list("t", "v"))
