...
res.multi.method.4 <- nmf(eset.corr, 4, list("brunet", "KL", "lee","nsNMF"), nrun=TRAIN, seed = 123456, .options = "t")
...
res.final.4 <- nmf(eset.corr, 4, "nsNMF", nrun=RUN, .options = "t")
```
...
consensusmap(res.final.4,  annCol=list(samplegroups=as.character(unlist(eset.corr$group_short ))),  labCol=groups.corr, labRow=groups.corr, annColors=list(samplegroups=cbPalette[1:4], basis=cbPalette[5:8], consensus=brewer.pal(4, "Spectral")))

coefmap(res.final.4, labCol=groups.corr, annColors=list(basis=cbPalette[5:8], consensus=brewer.pal(4, "Spectral"))) # shows the individual genes in relation to metagenes and samples

basismap(res.final.4, subsetRow=TRUE,annColors=list(basis=cbPalette[5:8], consensus=brewer.pal(4, "Spectral")), main="Metagene Components - All Contributing Genes") # shows metagenes in relation to samples
basismap(res.final.4, subsetRow=0.5,annColors=list(basis=cbPalette[5:8], consensus=brewer.pal(4, "Spectral")), main="Metagene Components - Most Specific Genes") # shows metagenes in relation to samples
...
metagene4.1 <- cbind(fData(eset.corr)[f4[[1]],], fs4[f4[[1]]]) 
names(metagene4.1)[ncol(metagene4.1)] <- "featurescore"
metagene4.2 <- cbind(fData(eset.corr)[f4[[2]],], fs4[f4[[2]]]) 
names(metagene4.2)[ncol(metagene4.2)] <- "featurescore"
metagene4.3 <- cbind(fData(eset.corr)[f4[[3]],], fs4[f4[[3]]]) 
names(metagene4.3)[ncol(metagene4.3)] <- "featurescore"
metagene4.4 <- cbind(fData(eset.corr)[f4[[4]],], fs4[f4[[4]]]) 
...
metagene.lsc.gvis <- gvisTable(as.data.frame(apply(metagene4.1, 2, as.character)), options = list(width = 960, height=800))  
...
genes.of.interest.lsc <- unique(unlist(lapply(metagene4.1$GeneName, function(x) unlist(strsplit(x, split="/"))[1])))
...
```{r PM.metagene.print, echo=FALSE, results='asis'}
metagene.PM.gvis <- gvisTable(as.data.frame(apply(metagene4.4, 2, as.character)), options = list(width = 960, height=800))  
print(metagene.PM.gvis, "chart")
```

#### Gene Ontology Analyses of PM metagene features

```{r GOPM, results='hide'}
# split up genes in list, when multiple genes
genes.of.interest.PM <- unique(unlist(lapply(metagene4.4$GeneName, function(x) unlist(strsplit(x, split="/")))))
background.genes.PM <- unique(unlist(lapply(fData(eset.corr)[,"GeneName"], function(x) unlist(strsplit(x, split="/")))))
...
```{r BMM.metagene.print, echo=FALSE, results='asis'}
metagene.BMM.gvis <- gvisTable(as.data.frame(apply(metagene4.3, 2, as.character)), options = list(width = 960, height=800))  
print(metagene.BMM.gvis, "chart")
```

#### Gene Ontology Analyses of BMM metagene features

```{r GOBMM, results='hide'}
# split up genes in list, when multiple genes
genes.of.interest.BMM <- unique(unlist(lapply(metagene4.3$GeneName, function(x) unlist(strsplit(x, split="/")))))
background.genes.BMM <- unique(unlist(lapply(fData(eset.corr)[,"GeneName"], function(x) unlist(strsplit(x, split="/")))))
