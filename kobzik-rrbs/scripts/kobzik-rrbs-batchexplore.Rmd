...
#### Methylation differeces at all sites, either batch  {.tabset}
...
#### Methylation differences at significantly differentially methylated sites, either batch {.tabset}
...
```{r methdiffcompare_rawdatasetsup, eval=TRUE, cache=FALSE}
...
#### Raw data values at all sites, either batch {.tabset}

```{r methdiffcompare_rawdataplotsi1}
ggplot(meth.quant.mat.m, aes(x=value, color=batch))+geom_density()+facet_wrap(~variable, scales="free",ncol=5)+scale_x_log10()+ggtitle("all samples, all sites")
ggplot(filter(meth.quant.mat.m, generation=="cntr"), aes(x=value, color=batch))+geom_density()+facet_wrap(~variable,ncol=5,scales="free")+scale_x_log10()+ggtitle("control samples, all sites")
ggplot(filter(meth.quant.mat.m, generation=="F1"), aes(x=value, color=batch))+geom_density()+facet_wrap(~variable,ncol=5,scales="free")+scale_x_log10()+ggtitle("F1 samples, all sites")
```
