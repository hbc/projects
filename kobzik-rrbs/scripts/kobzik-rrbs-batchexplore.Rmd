...
```{r logreg, eval=FALSE}
#batch1
b1F1.diffMeth <- calculateDiffMeth(meth.quants.b1F1cntr,slim=TRUE,  weighted.mean=TRUE,num.cores=2)
b1F2.diffMeth <- calculateDiffMeth(meth.quants.b1F2cntr,slim=TRUE,  weighted.mean=TRUE,num.cores=2)
b1F3.diffMeth <- calculateDiffMeth(meth.quants.b1F3cntr,slim=TRUE,  weighted.mean=TRUE,num.cores=2)

#batch2
b2F1.diffMeth <- calculateDiffMeth(meth.quants.b2F1cntr,slim=TRUE,  weighted.mean=TRUE,num.cores=2)
b2F2.diffMeth <- calculateDiffMeth(meth.quants.b2F2cntr,slim=TRUE,  weighted.mean=TRUE,num.cores=2)
b2F3.diffMeth <- calculateDiffMeth(meth.quants.b2F3cntr,slim=TRUE,  weighted.mean=TRUE,num.cores=2)
...
meth.quant.mat.betas.m <- inner_join(numCs.m, numTs.m, by=c("chr", "start", "end", "strand", "samplid", "samplenum", "batch", "generation")) %>%
  mutate(., beta=value.x/(value.x+value.y)) %>%
  mutate(., M=log2((value.x+1e-5)/(value.y+alpha))) %>%
  dplyr::select(., -contains("variable"), -contains("value")) %>%
  reshape2::melt(., id.vars=c("chr", "start", "end", "strand","samplid", "samplenum", "batch", "generation")) %>% 
  tbl_df() %>% 
  dplyr::select(., chr, start, end, strand, variable, value, samplenum, samplid, batch, generation)
...
```{r methdiffcompare_rawdataload, echo=FALSE}
load(file.path(resultsDir, "melted.methylkit.data"))
```
...
ggplot(meth.quant.mat.betas.m, aes(x=beta, color=batch))+geom_density()+facet_wrap(~generation)+ggtitle("Beta values - all sites")
ggplot(meth.quant.mat.betas.m, aes(x=M, color=batch))+geom_density()+facet_wrap(~generation)+ggtitle("M values - all sites")
