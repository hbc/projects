```{r setup, echo=FALSE}
opts_chunk$set(tidy=TRUE, cache=FALSE,  highlight=TRUE, figalign="center", echo=TRUE, warning=FALSE, error=FALSE, message=FALSE, tidy.opts=list(keep.blank.line=FALSE, width.cutoff=120), fig.path="figure/WB-U133Plus2-barcode-", cache.path="cache/WB-U133Plus2-barcode-")
options(width=200)
```
...
```{r barcode_load_affybatch, echo=FALSE, eval=FALSE}
...
```{r barcode_frma_run, cache=TRUE, eval=FALSE}
...
**The improvement from frma normalization is striking.**
...
for (n in 1:3) {
  ...
  if (nrow(bc.contrast.sub)>0) {
    ...
    pheatmap(bc.contrast.sub, 
             ...
             legend_breaks=c(0,1), 
             main=paste("Barcoded Intensities for ", contrast, " comparison - Unclustered",  sep=""))
    }
  }
```
...
These looser cutoffs find a very large number of genes, being more stringent in our cutoffs will reduce the number of genes to a manageable level. After experimenting with different cutoffs, the upper limit for the inter-group entropy is 0.3, anything higher fails to yield any genes. However, we can reduce the intra-group entropy (i.e. how consistent the group is within itself) to 0.2 before we stop seeing results.
...
[Neonate vs. Child](../results/WB/U133Plus2/U133_2.0Plus.barcode.entropy.cutoff.0.25.neonate-child.xls)

```{r barcode_filters3}
intra.entropycutoff <- 0.22
inter.entropycutoff <- 0.30
```

We can reduce the intra-group entropy even further to  (<`r intra.entropycutoff`).

```{r barcode_diff_heatmap0.2, ref.label='barcode_diff_heatmap0.3', fig.width=18, fig.height=18}
```

#### Excel files with probeIDs, gene symbols, entropy values and barcoded expression values for each comparison

[Adult vs. Child](../results/WB/U133Plus2/U133_2.0Plus.barcode.entropy.cutoff.0.22.adult-child.xls)  
[Adult vs. Neonate](../results/WB/U133Plus2//U133_2.0Plus.barcode.entropy.cutoff.0.22.adult-neonate.xls)  
[Neonate vs. Child](../results/WB/U133Plus2/U133_2.0Plus.barcode.entropy.cutoff.0.22.neonate-child.xls)
