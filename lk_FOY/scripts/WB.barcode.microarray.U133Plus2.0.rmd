...
```{r barcode_load_affybatch, eval=TRUE}
load(file.path(resultsDir, "RDATA.mic.norm.no.outliers"))
load(file.path(resultsDir, "RDATA.mic.raw.no.outliers"))
```
...
# annotation colors
pd <- pData(mic.frma)
study_colors <- c(cbPalette, "#000000", "#FF0000")
...
for (n in 1:3) {
  ...
  if (nrow(bc.contrast.sub)>0) {
    ...
    plot.new()
    svg(filename=file.path(resultsDir, paste("barcode.heatmap.0.3", contrast, "svg", sep=".")), width=8, height=11)
...
for (n in 1:3) {
  ...
  if (nrow(bc.contrast.sub)>0) {
    ...
    svg(filename=file.path(resultsDir, paste("barcode.heatmap.0.29", contrast, "svg", sep=".")), width=8, height=11)
    plot.new()
    pheatmap(bc.contrast.sub, 
             color=c("blue", "red"), 
             cluster_rows=TRUE, 
...
![adult-child barcode heatmap](../results/WB/U133Plus2/barcode.heatmap.0.29.adult-child.svg)
![adult-neonate barcode heatmap](../results/WB/U133Plus2/barcode.heatmap.0.29.adult-neonate.svg)
![neonate-child barcode heatmap](../results/WB/U133Plus2/barcode.heatmap.0.29.neonate-child.svg)


#### Excel files with probeIDs, gene symbols, entropy values and barcoded expression values for each comparison

[Adult vs. Child](../results/WB/U133Plus2/U133_2.0Plus.barcode.entropy.cutoff.0.29.adult-child.xls)  
[Adult vs. Neonate](../results/WB/U133Plus2//U133_2.0Plus.barcode.entropy.cutoff.0.29.adult-neonate.xls)  
[Neonate vs. Child](../results/WB/U133Plus2/U133_2.0Plus.barcode.entropy.cutoff.0.29.neonate-child.xls)
...
for (n in 1:3) {
  ...
  if (nrow(bc.contrast.sub)>0) {
    ...
    svg(filename=file.path(resultsDir, paste("barcode.heatmap.0.275", contrast, "svg", sep=".")), height=11, width=8)
    plot.new()
    pheatmap(bc.contrast.sub, 
             color=c("blue", "red"), 
             cluster_rows=TRUE, 
...
![adult-child barcode heatmap](../results/WB/U133Plus2/barcode.heatmap.0.275.adult-child.svg)
![adult-neonate barcode heatmap](../results/WB/U133Plus2/barcode.heatmap.0.275.adult-neonate.svg)


#### Excel files with probeIDs, gene symbols, entropy values and barcoded expression values for each comparison

[Adult vs. Child](../results/WB/U133Plus2/U133_2.0Plus.barcode.entropy.cutoff.0.275.adult-child.xls)  
[Adult vs. Neonate](../results/WB/U133Plus2//U133_2.0Plus.barcode.entropy.cutoff.0.275.adult-neonate.xls)  
