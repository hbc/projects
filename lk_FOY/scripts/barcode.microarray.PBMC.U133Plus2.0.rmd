...
Used the same samples as the previous limma [analysis][6]
...
```{r barcode_clustering, fig.width=12, fig.height=12}
...
Same setup as for the previous limma [analysis][6]
...
### Top 100 differentially expressed genes from the [previous][6] standard non-barcoded, RMA normalized, limma  data
- for comparison 
...
```{r barcode_heatmaps, fig.height=18, fig.width=18}
for(n in 1:3) {
  ...
  ## Heatmaps
  # both genders
  pheatmap(as.matrix(top.barcodes.contrast), color=c("blue", "red"), annotation=heatmap.annots, annotation_colors=ann_colors, legend_breaks=c(0,1), main=contrast, show_colnames=F, fontsize=18, fontsize_row=10, cluster_cols=T, cluster_rows=T) 
  # female gender
  top.barcodes.contrast.female <- top.barcodes.contrast[,which(pData.contrast$gender=="FEMALE")]
  pheatmap(as.matrix(top.barcodes.contrast.female), color=c("blue", "red"), annotation=subset(heatmap.annots, gender=="FEMALE"), main=paste(contrast, "(FEMALE)", sep=" "),show_colnames=F, fontsize=18, fontsize_row=10,legend_breaks=c(0,1), annotation_colors=ann_colors, cluster_cols=T, cluster_rows=T) 
  # male gender
  top.barcodes.contrast.male <- top.barcodes.contrast[,which(pData.contrast$gender=="MALE")]
  pheatmap(as.matrix(top.barcodes.contrast.male), color=c("blue", "red"), annotation=subset(heatmap.annots,gender=="MALE"), main=paste(contrast, "(MALE)", sep=" "),show_colnames=F, fontsize=18, fontsize_row=10, annotation_colors=ann_colors, legend_breaks=c(0,1), cluster_cols=T, cluster_rows=T) 
  }
# cleanup
...
Once again, there are some genes that cluster well by age group, but also many that are homogenous across age groups. In cases that cluster gy age group, it is difficult to separate study specific effects from age group effects.
...
```{r barcode_diff_heatmap0.3, fig.width=18, fig.height=18}
...
for (n in 1:3) {
  ...
  write.table(output, file.path(resultsDir, paste("U133_2.0Plus.PBMC.barcode.entropy.cutoff", intra.entropycutoff, contrast, "xls", sep=".")), sep="\t", row.names=F, col.names=T)
  ## Heatmaps
  ...
  # both genders
  pheatmap(bc.contrast.sub, color=c("blue", "red"), cluster_rows=TRUE, cluster_cols=T, show_colnames=F, show_rownames=showrownames_var, fontsize=18, fontsize_row=10, annotation=heatmap.annots, annotation_colors=ann_colors, legend_breaks=c(0,1), main=paste("Barcoded Intensities for ", contrast, " comparison",  sep=""))
  # female only
  bc.contrast.sub.female <- bc.contrast.sub[,which(pData.contrast$gender=="FEMALE")]
  pheatmap(bc.contrast.sub.female, color=c("blue", "red"), cluster_rows=TRUE, cluster_cols=TRUE, show_colnames=F, show_rownames=showrownames_var, annotation=subset(heatmap.annots, gender=="FEMALE"), legend_breaks=c(0,1), main=paste("Barcoded Intensities for ", contrast, " comparison (FEMALE)",  sep=""), fontsize=18, fontsize_row=8, annotation_colors=ann_colors)
  # male only
  bc.contrast.sub.male <- bc.contrast.sub[,which(pData.contrast$gender=="MALE")]
  pheatmap(bc.contrast.sub.male, color=c("blue", "red"), cluster_rows=TRUE, cluster_cols=TRUE, show_colnames=F, show_rownames=showrownames_var,annotation=subset(heatmap.annots, gender=="MALE"), legend_breaks=c(0,1), main=paste("Barcoded Intensities for ", contrast, " comparison (MALE)",  sep=""), fontsize=18, fontsize_row=8, annotation_colors=ann_colors)
  }
```

#### Excel files with probeIDs, gene symbols, entropy values and barcoded expression values for each comparison

[Adult vs. Child](./results/U133_2.0Plus.PBMC.barcode.entropy.cutoff.0.3.adult-child.xls)  
[Adult vs. Neonate](./results/U133_2.0Plus.PBMC.barcode.entropy.cutoff.0.3.adult-neonate.xls)  
[Neonate vs. Child](./results/U133_2.0Plus.PBMC.barcode.entropy.cutoff.0.3.neonate-child.xls) 
...
```{r barcode_diff_heatmap0.25, ref.label='barcode_diff_heatmap0.3', fig.width=18, fig.height=18}
```

#### Excel files with probeIDs, gene symbols, entropy values and barcoded expression values for each comparison

[Adult vs. Child](./results/U133_2.0Plus.barcode.entropy.cutoff.0.275.adult-child.xls)  
[Adult vs. Neonate](./results/U133_2.0Plus.barcode.entropy.cutoff.0.275.adult-neonate.xls)  
[Neonate vs. Child](./results/U133_2.0Plus.barcode.entropy.cutoff.0.275.neonate-child.xls)
