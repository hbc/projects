---
title: "Transgenic vs Knockout Differential Expression - Young samples"
...
deDir <- file.path(outputDir, "results", "differential_expression",  "KOvsTG", "young")
...
dds <- DESeqDataSetFromTximport(
    ...
    colData = colData(bcb),
    design = design) 
dds$generalGeno <- relevel(dds$generalGeno, ref="WT")
dds <- DESeq(dds)
rld <- rlog(dds)
```
...
```{r gprofiler_revigoWTKO, results='asis'}
run_gprofiler_revigo(my_results=res_WTKO)
```

### TG versus WT

```{r gprofiler_revigoTGWT, results='asis'}
run_gprofiler_revigo(my_results=res_TGWT)
```

### TG versus KO 

```{r gprofiler_revigoTGKO, results='asis'}
run_gprofiler_revigo(my_results=res_TGKO)
```


# Genes that go in opposite directions in transgenics and knockouts

Find genes that have a significant difference between trangenics and knockouts with opposite direction LFCs in their respective wildtype comparisons.

Note that these genes do not necessarily have significant differences between wildtype samples and trangenics or between wildtype samples and knockouts. See the table below for the stats , where a TGWT suffix denotes the transgenic versus wildtype comparison (positive equals higher expression in the transgenic) and WTKO denotes the wildtype versus knockout comparison (positive equals higher expression in the wildtype).

```{r oppoTGKO, results='asis'}
sig_TGKO <- res_TGKO %>% as.data.frame() %>%  rownames_to_column(., var="ensemblid") %>% tbl_df %>% filter(padj<alpha)



res_TGWT <-  res_TGWT %>% as.data.frame() %>%  rownames_to_column(., var="ensemblid") %>% tbl_df
res_WTKO <-  res_WTKO %>% as.data.frame() %>%  rownames_to_column(., var="ensemblid") %>% tbl_df 

temp <- inner_join(res_TGWT, res_WTKO, by="ensemblid", suffix=c(".TGWT", ".WTKO"))

sig_TGKO <- inner_join(sig_TGKO, temp, by="ensemblid")
sig_TGKO <- inner_join(sig_TGKO, metadata(bcb)$annotable, by=c("ensemblid"="ensgene"))

kable(sig_TGKO)

plotGene(bcb, sig_TGKO$ensemblid, format="ensgene", normalized="tpm")
...
All files can be found in this [folder](./results/differential_expression/KOvsTG/young). 
