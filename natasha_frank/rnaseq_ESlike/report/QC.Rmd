...
```{r plot_pca, fig.height=12, fig.width=10, results='asis'}
plotPCA(bcb)

bcb_cornea <- bcb[,which(colData(bcb)$tissue=="cornea")]
bcb_skin <- bcb[,which(colData(bcb)$tissue=="skin")]
bcb_es <- bcb[,which(colData(bcb)$tissue=="es")]
bcb_total_cornea <- bcb[,which(colData(bcb)$tissue=="cornea" & colData(bcb)$facsSorted=="no")]
bcb_sorted_cornea <- bcb[,which(colData(bcb)$tissue=="cornea" & colData(bcb)$facsSorted=="yes")]

cat("### Skin samples only")
plotPCA(bcb_skin)
cat("### ES cell samples only")
plotPCA(bcb_es)
cat("### Cornea samples only")
plotPCA(bcb_cornea)
cat("### FACS sorted corena samples only")
plotPCA(bcb_sorted_cornea)
cat("### Total cornea samples only")
plotPCA(bcb_total_cornea)
```
