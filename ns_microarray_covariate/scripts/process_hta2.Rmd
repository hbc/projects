...
[Normalized Data QC Report](../results/report_norm.exon/index.html)
...
```{r}
# Make design matrix
pData(affyNorm)$blooddonor <- factor(pData(affyNorm)$blooddonor)
design <- model.matrix(~ 0 + treatment + blooddonor  ,  data=pData(affyNorm))
kable(design)
```
...
```{r, fig.align='center'}
results <- topTable(fit2.core, coef=1, number=nrow(exprs(affyNorm)))
...
affyNorm.filt.b2 <- affyNorm.b2[which(!is.na(fData(affyNorm.b2)$symbol) & fData(affyNorm.b2)$category=="main"),]
```
