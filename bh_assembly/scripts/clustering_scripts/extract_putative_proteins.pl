...
open FILE, $mfa or die print STDERR "Could not open file $mfa corresponding to input $file!\n";
...
if (-e "all.strains.prod.train") {
	...
	die print STDERR "Could not use Prodigal training file all.strains.prod.train\n";
}

open PROD, "$file.prod.tab" or die print STDERR "Prodigal gene prediction on $file failed!\n";
...
open PRED, "$file.glim.predict" or die print STDERR "Glimmer3 gene prediction on $file failed!\n";
open RED, "> filtered.$file.glim.predict" or die print STDERR "Could not write data to file filtered.$file.glim.predict\n";
...
foreach (<PRED>) {
	...
	if (/^>/) {
		...
		if ($data[3] > 0) {
			...
			foreach $p (keys %prodigal_start) {
				if ($prodigal_strand{$p} == 1) {
					my $pob1 = $prodigal_start{$p}+0.25*($prodigal_end{$p} - $prodigal_start{$p});	# look for overlap in the central 50%, ignore overlaps that only involve the ends
					my $pob2 = $prodigal_end{$p}-0.25*($prodigal_end{$p} - $prodigal_start{$p});
						if (($pob1 >= $gob1 && $pob1 <= $gob2) || ($pob2 >= $gob1 && $pob2 <= $gob2)) {
							if (($prodigal_end{$p} - $prodigal_start{$p}) < ($data[2] - $data[1])) {
								$comp_start = $data[1];
								$comp_end = $data[2];
							} else {
		...
		if ($prodigal_match == 1) {
			if ($broken == 0) {
				if ($data[3] > 0) {
					print RED "$data[0]\t$comp_start\t$comp_end\t$data[3]\t$data[4]\n";
				} else {
					print RED "$data[0]\t$comp_end\t$comp_start\t$data[3]\t$data[4]\n";
				}
			} else {
				if ($data[3] > 0) {
					my $new_end = $broken-1;
					if (abs(1+$new_end-$comp_start) >= 99) {
						print RED "$data[0]\t$comp_start\t$new_end\t$data[3]\t$data[4]\n";
					}
				} else {
					my $new_end = $broken;
					if (abs(1+$new_end-$comp_end) >= 99) {
						print RED "$data[0]\t$comp_end\t$new_end\t$data[3]\t$data[4]\n";
					}
				}
			}
		}
...
close SEQ;
close HEAD;
system "grep -v 'ID' $tab | sed s/label=\"orf/label=\"seq"."$count"."orf/g | cat $header - $emblseq > $embl";
#system "rm $file.glim*";
...
my $DNA_outfile = Bio::SeqIO->new( -format => 'Fasta', -file => "> $DNA_outname");
for my $gene ($annotation->get_SeqFeatures) {	
		...
		for my $name ($gene->get_tag_values('label')) {
			...
			$DNA_string =~ s/R|Y|W|S|M|K|H|B|D|V|X/N/g;	# deal with ambiguity codes
...
open REP, "> $file.report" or die print STDERR "Cannot open output file $file.report\n";
