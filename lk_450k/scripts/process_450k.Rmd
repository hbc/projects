...
metadata <- read.xlsx(file.path(metaDir, "human-methylation450-96samples-sample-sheet_Kobzik.xlsx"), sheetIndex = 2, header=F)

# fix the name of the renamed replicate sample in the samplesheet so you can match to metadata
samplesheet$Sample.ID <- sub("a5_2", "a5",samplesheet$Sample.ID)
...
#minfi 
## method, use etended option to get an RGChannelSEtextended, necessary for pfiltering in next steps
...
all.stats <- apply(selected.combos, 2, function(combo){
  ...
  stats.annot <- merge(stats, as.data.frame(annots), by.y="TargetID")
