...
arraytype="GeneChip® Human Transcriptome Array 2.0"
...
So far we have only been working with the probesets,without reference to the exons and genes they assay. Here we load in metadata about the probesets on the array (feature data), the gene symbols in particular.

USE THE AFFY ANNOTATIONS INSTEAD

```{r features, results='hide'}
featureData(affyNorm.gene) <- getNetAffx(affyNorm.gene, "transcript") 
featureData(affyNorm.exon) <- getNetAffx(affyNorm.exon, "probeset") 
# this will load the Affymetrix annotation, including the probeID, into the fData
```

## Filter Probesets
Reducing the number of exons and genes assayed reduces  the multiple test correction and may allow us to identify more differentially spliced genes.

The gene summarized dataset starts with `r nrow(affyNorm.gene)[[1]]` probesets/genes.

The exon summarized dataset starts with `r nrow(affyNorm.exon)[[1]]` probesets/exons.

### By Annotation
- remove the probesets with no annotations

```{r filter1}
affyNorm.gene <- affyNorm.gene[!apply(fData(affyNorm.gene), 1, function(x) all(is.na(x))),]
fdata.gene <- fData(affyNorm.gene) %>% tbl_df()

affyNorm.exon <- affyNorm.exon[!apply(fData(affyNorm.exon), 1, function(x) all(is.na(x))),]
fdata.exon <- fData(affyNorm.exon) %>% tbl_df()

 apply()
unlist(strsplit(fdata.gene$geneassignment[3], "\\/\\/\\/"))
```

Filter out any probesets that are known to assay multiple genes


```{r getgenes}
# get gene symbols and entrezIDs for all probesets 
fData(affyNorm.gene)$symbol <- as.character(unlist(mget(featureNames(affyNorm.core), mogene10sttranscriptclusterSYMBOL, ifnotfound=NA))) # curated annotations from Bioconductor 
